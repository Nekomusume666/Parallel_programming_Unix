...
void initialize_matrices(
    ...
    std::mt19937 gen(1234); // Генератор случайных чисел
    std::uniform_int_distribution<> dis(1, 10); // Диапазон значений (1-10)
    ...
    // Транспонируем матрицу B и сохраняем в BT
...
// Функция умножения матриц с 20 процессами (MPI)
void multiply_matrices_mpi_20(int rank, int size) {
    ...
    if (rank == 0) {
        initialize_matrices(A, B, BT); // Инициализация матриц
    }
    ...
    for (int i = 0; i < COLS_B; i++) {
        ...
        if (rank % GROUP_SIZE == 0) {
            ...
            std::array<int, COLS_B> temp_result = result[row_idx];
            MPI_Gather(temp_result.data(), COLS_B, MPI_INT, result.data(), COLS_B, MPI_INT, 0, leader_comm);
        }
    }
...
// Функция умножения матриц с 4 процессами (MPI)
    ...
    for (int j = 0; j < COLS_B; j++) {
        ...
        for (int k = 0; k < COLS_A; k++) {
            ...
            dot_product += my_a_row[k] * bt_col[k];
             if (rank == 0)
            {
                std::cout << "Rank " << rank << ", j=" << j << ", k=" << k
               << " | my_a_row[" << k << "]=" << my_a_row[k]
               << " * bt_col[" << k << "]=" << bt_col[k]
               << "        res =" << dot_product
               << std::endl; 
            }
